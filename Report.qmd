...
geom_point() +
labs(x = "1/Eletricity Demand (1/TWh)", y = "Life Expectancy (years)")
```
...
**Conjunto II:** `continent`, `gdp_pc`, `no_water` e `urban_population_percent` (e/ou transformações)
...
O conjunto II será escolhido:

```{r}
filter_owid <- owid %>% filter(year == 2015, continent != "Oceania")
filter_owid <- filter_owid[, c(1, 17, 18, 20, 21)]
...
Iremos considerar uma relação linear de `life_expectancy` com `no_water` e `urban_population_percenet`. É preciso investigar a relação com `gdp_pc`.

```{r}
ggplot(filter_owid, aes(y = life_expectancy, x = log(gdp_pc))) +
geom_point() +
labs(x = "log(GDP)", y = "Life Expectancy (years)")
```

A relação de `log(gdp_pc)` e `life_expectancy` parece ser linear, essa alteração será feita para a construção do modelo.

```{r}
filter_owid <- filter_owid %>% mutate(log_gdp_pc = log(gdp_pc))
filter_owid <- filter_owid[, c(1, 3, 4, 5, 6)]
summary(filter_owid)
```

```{r}
cor(filter_owid$no_water, filter_owid$no_water^2)
cor(filter_owid$urban_population_percent, filter_owid$urban_population_percent^2)
cor(filter_owid$log_gdp_pc, filter_owid$log_gdp_pc^2)
```

centralização das variáveis explicativas

```{r}
filter_owid <- filter_owid %>%
  mutate(no_water_c = no_water - mean(no_water), 
         urban_population_c = urban_population_percent - mean(urban_population_percent),
         log_gdp_c = log_gdp_pc - mean(log_gdp_pc))

cor(filter_owid$no_water_c, filter_owid$no_water_c^2)
cor(filter_owid$urban_population_c, filter_owid$urban_population_c^2)
cor(filter_owid$log_gdp_c, filter_owid$log_gdp_c^2)
```

A correção de `no_water_c` ainda está, das demais variáveis foi reduzida.
...
```{r}
m_quant <- lm(life_expectancy ~ I(no_water_c^2) + I(urban_population_c^2) + I(log_gdp_c^2) + no_water_c * urban_population_c + no_water_c * log_gdp_c + urban_population_c * log_gdp_c, data = filter_owid)
...
O modelo passou no teste-F global, não há grandes diferenças entre $R^2$ e $R^2$ ajustado. Os termos quadráticos não são significativos, como esperado. Além disso, os termos de interação e `urban_population_c` também não parecem ser significativos.

```{r}
m_quant_red <- lm(life_expectancy ~  no_water_c + log_gdp_c, data = filter_owid)

summary(m_quant_red)
```
O modelo reduzido é significativo. Vamos realizar o teste linear geral.

```{r}
anova(m_quant_red, m_quant, test = "F")
```   

O valor-p é menor que 0.05, logo é possível rejeitar a hipótese nula.

Vamos adicionar `urban_population_c` e o termo de intereção entre `no_water_c` `log_gdp_c` ao modelo. Esses termos serão adicionados por serem os que possuem menor valor-p, que estão acima de 0.05.

```{r}
m_quant_red <- lm(life_expectancy ~ urban_population_c + no_water_c * log_gdp_c, data = filter_owid)
...
Veja que agora o valor-p é maior que 0.05, logo não é possível rejeitar a hipótese nula. Dessa forma, é possível utilizar o modelo reduzido com segurança dessa decisão.

A agora será a realizado a análise de resíduos.

```{r}
# Constrói tabela com dados do modelo `m_quant_red`
owid_m_quant_red_data <- filter_owid %>%
  # inclui coluna com valores ajustados
  mutate(fitted = m_quant_red$fit) %>%
  mutate(resid = m_quant_red$res)

# Gera gráficos dos resíduos:
ggplot(owid_m_quant_red_data, aes(x = fitted, y = resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  ggtitle("gráfico de resíduos: modelo life_expectancy ~ urban_population_c + no_water_c * log_gdp_c") +
  labs(y = "resíduos", x = "resposta ajustada (life_expectancy)")
# Gráfico de quantis
ggplot(owid_m_quant_red_data, aes(sample = resid)) +
  stat_qq() + stat_qq_line() +
  ggtitle("gráfico de quantis normais") +
  labs(y = "quantis amostrais", x = "quantis teóricos (dist. normal)") 
```

É possível ver uma tendência não linear no gráficos dos resíduos, uma transformação da resposta ajustada possa resolver esse problema. As outras hipóteses não parecem ter sido violadas.

Há uma certa fuga da normalidade, pelo gráfico dos quantis normais, mas sem grandes devios.
...
shapiro.test(owid_m_quant_red_data$resid)
```

Os valores-p de ambos os testes são abaixo de 0.05, então as hipóteses nulas podem ser rejetitadas. Então a variância dos resíduos não é constante e há violação da normalidade.

```{r}
bc <- boxcox(m_quant_red, 
             lambda = seq(-10, 10, by = 0.1), plotit = TRUE)
```
...
O valor de $\lambda$ que maximiza a função de verossemelhança vale `r round(lambda, digits = 2)`, ou seja, a transformção Box-Cox sugere realizar a transformação $Y' = Y^3$.

```{r}
filter_owid <-  filter_owid %>%
  mutate(life_expectancy_bc = life_expectancy^3)

bc_model <- lm(life_expectancy_bc ~ urban_population_c + no_water_c * log_gdp_c, data = filter_owid)
...
A tendência não linear dos resíduos aparenta ser sumido.
Ainda há violações da normalidade perto das pontas da distribuição.
...
Os valores-p dos testes não se alteram muito, indicando que ainda há violação da normalidade e que a variância não é constante.

2. inclusão de variáveis qualitativas e suas interações;

Como há apenas uma variável qualitativa, não haverá adicição de termos de intereção no momento.

```{r}
m_quali <- lm(life_expectancy ~ urban_population_c + no_water_c * log_gdp_c + continent, data = filter_owid)

summary(m_quali)
```

Pela estatística F do teste global, o modelo continua signifivativo. Além disso, todas as variáveis *dummies* associadas a `continent` também são significativas, pelo valor-p do test-t.

```{r}
anova(m_quali)
```

Veja que há significância incremental de todos os termos, a pesar de haver uma redução, que é esperado, mas todos os testes-F possuem valor significativo.

3. inclusão de interações entre termos contendo variáveis quantitativas e qualitativas;

```{r}
ggplot(filter_owid, aes(x = urban_population_c, y = life_expectancy)) +
  geom_point(aes(color = continent, shape = continent)) + 
  geom_smooth(aes(color = continent), method = "lm", se = FALSE) +
  ggtitle("`life_expectancy` x `urban_population_c`, para diferentes níveis de `continent`")
ggplot(filter_owid, aes(x = no_water_c, y = life_expectancy)) +
  geom_point(aes(color = continent, shape = continent)) + 
  geom_smooth(aes(color = continent), method = "lm", se = FALSE) +
  ggtitle("`life_expectancy` x `no_water_c`, para diferentes níveis de `continent`")
ggplot(filter_owid, aes(x = log_gdp_c, y = life_expectancy)) +
  geom_point(aes(color = continent, shape = continent)) + 
  geom_smooth(aes(color = continent), method = "lm", se = FALSE) +
  ggtitle("`life_expectancy` x `log_gdp_c`, para diferentes níveis de `continent`")
```

Pelos gráficos não parece haver uma forte relação entre `urban_population_c` e `continent`. O mesmo ococrre para `log_gdp_c`. Para `no_water_c` parece haver uma mudança da inclinação. Isso será melhor analisados com as estatísticas apropriadas.

```{r}
m_all <- lm(life_expectancy ~ no_water_c * log_gdp_c +  urban_population_c * continent + continent * no_water_c + log_gdp_c * continent, data = filter_owid)

summary(m_all)
```

Pelo teste-F global o modelo é significativo, mas houve uma redução da estatística $R^2$ ajustado. Nenhum termo de interação adicional é significativo.

```{r}
anova(m_all)
```

Veja que há pouquíssima adição dos termos de interação com `continent` e as demais variáveis explicativas quantitativas. Vamos utilizar o teste linear geral para avaliar a significância desses termos adicionais.

```{r}
anova(m_quali, m_all, test = "F")
```  

Veja que o valor-p da estatística-F é quase 1, logo não é possível rejeitar a hipótese nula. Dessa forma, não serão adicionados os termos de interação de `continent` com as demais variáveis quantitativas.

**avaliação do modelo**

+ realizar análise de resíduos (verificar validade das hipóteses)

Como os termos de interação não foram adicionados, iremos analisar o modelo `m_quali`.

```{r}
# Constrói tabela com dados do modelo `m_quali`
owid_m_quali_data <- filter_owid %>%
  # inclui coluna com valores ajustados
  mutate(fitted = m_quali$fit) %>%
  mutate(resid = m_quali$res) 
```

```{r}
# Gera gráficos dos resíduos:
ggplot(owid_m_quali_data, aes(x = fitted, y = resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  ggtitle("gráfico de resíduos: modelo life_expectancy ~ urban_population_c + no_water_c * log_gdp_c + continent") +
  labs(y = "resíduos", x = "resposta ajustada (life_expectancy)")
```

```{r}
# Gráfico dos resíduos padronizados
ggplot(owid_m_quali_data, aes(x = fitted, y = resid/sd(resid))) +
  geom_point() +
  geom_hline(yintercept = c(-4, -3, 0, 3, 4), 
             linetype = "dashed", 
             color = c("red", "orange", "black", "orange", "red") ) +
  ylim(-6, 6) +
  ggtitle("gráfico de resíduos padronizados: modelo life_expectancy ~ urban_population_c + no_water_c * log_gdp_c + continent") +
  labs(y = "resíduos padronizados", x = "resposta ajustada (life_expectancy)")
```

O gráfico dos resíduos mostra uma aparente heterocedasticidade, pois há uma tendência de diminução da variância dos resíduos conforme o aumento da resposta ajustada. O gráfico dos resíduos padronizados mostra que há apenas um ponto extremo, pois está fora do intervalo $[-3, 3]$.

Para constatar a heterocedasticidade, iremos realizar o teste de Breusch-Pagan.

```{r}
bptest(m_quali)
```

O Teste deu um p-valor de 0.0086, o que da evidência estatistica suficiente para rejeitar a hipótese nula de que a variância dos resíduos é constante. Dessa forma, há heterocedasticidade.

Esse erro de modelo não é tão grave, pois o modelo ainda é significativo e a heterocedasticidade não é tão grande, por enquanto iremos continuar analisando o modelo.

Vamos agora analisar a normalidade dos resíduos.

```{r}
# Gráfico de quantis
ggplot(owid_m_quali_data, aes(sample = resid)) +
  stat_qq() + stat_qq_line() +
  ggtitle("gráfico de quantis normais") +
  labs(y = "quantis amostrais", x = "quantis teóricos (dist. normal)") 
```

O gráfico aparenta ser de uma distribuição normal, mas há uma leve fuga nas pontas da distribuição. Vamos realizar o teste de Shapiro-Wilk.

```{r}
shapiro.test(owid_m_quali_data$resid)
```

O teste deu um p-valor de 0.83, isso indica que não há evidências estatísticas suficientes para rejeitar a hipótese nula de que os resíduos seguem uma distribuição normal.

Vamos analisar agora a linearidade do modelo.

Pelo gráfico de resíduos, não parece haver uma tendência não linear. Vamos realizar o teste de Ramsey RESET.

```{r}
resettest(m_quali)
```

O p-valor de 0.92 no teste de RESET sugere que não há evidências estatisticamente significativas para rejeitar a hipótese nula de que o modelo de regressão está especificado corretamente. Portanto, com base nos dados analisados, não há indícios convincentes de que haja omissão de variáveis relevantes ou problemas na forma funcional do modelo.

Essa conclusão sugere que o modelo atual pode ser apropriado para explicar a relação entre as variáveis incluídas, pelo menos com relação à detecção de não linearidades ou especificações inadequadas que o teste de RESET examina.

Por fim, iremos analisar a independência dos resíduos. A independência dos resíduos também pode ser entendido como um problema em que uma variável explicativa importante foi omitida do modelo, mas isso já foi analisado na análise de linearidade. Assim, para o teste de independência dos resíduos, iremos utilizar o teste de Durbin-Watson.

```{r}
dwtest(m_quali)
```

DW = 2.3472: O valor da estatística de Durbin-Watson é próximo a 2, sugerindo que não há forte evidência de autocorrelação nos resíduos.

p-valor = 0.98: O p-valor é bem próximo de 1, o que sugere que não há evidências estatísticas suficientes para rejeitar a hipótese nula de que não há autocorrelação nos resíduos.

Assim, concluimos que não há evidências de autocorrelação nos resíduos.

Como o modelo ainda é significativo, iremos continuar a análise.
...
O modelo final obtido foi o:

```{r}
summary(m_quali)
```

Formula: `life_expectancy ~ urban_population_c + no_water_c * log_gdp_c + continent`

Modelo: `Yhat = 66.984 + 0.003417 * urban_population_c + ...`

----

A partir dos coefientes gerados podemos perceber que continentes com maior população urbana possuem maior expectativa de vida, pois o coeficiente de `urban_population_c` é positivo e significativo. Além disso, o coeficiente de `no_water_c` é negativo e significativo, o que indica que quanto maior a porcentagem de pessoas sem acesso a água potável, menor a expectativa de vida. O coeficiente de `log_gdp_c` é positivo e significativo, o que indica que quanto maior o PIB per capita, maior a expectativa de vida.

Isso pode ser inferido a partir do modelo final gerado para a espectativa de vida em função das variáveis explicativas. Com isso podemos inferir que a expectativa de vida é maior em países com maior população urbana, maior PIB per capita e menor porcentagem de pessoas sem acesso a água potável e com esses indicadores é possível ajudar na tomada de desição para melhorar a expectativa de vida de um país.